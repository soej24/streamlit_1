...
-- 두개 테이블을 하나로 합쳐서 가져오시오. join 컬럼명 on 같은 컬럼이 되는것
select *
from customers
join orders
    on customers.id = orders.customer_id;
    
-- 테이블 이름을 줄여서 사용하는 방법(실무에서는 줄여서 사용한다)
select *
from customers c
join orders o
    on c.id = o.customer_id;
    
-- 실무에서 컬럼의 이름이 중복되는 경우는 컬럼이름을 바꿔서 가져와야 한다.
select c.id as customer_id, c.first_name, c.email, o.id as order_id, o.amount, o.order_date
from customers c
join orders o
    on c.id = o.customer_id;
    
-- 지금까지 위에서 한 join은 두개 테이블에 공통으로 들어있는 데이터만 가지고 온 것이다.
...
-- 주문 금액이 600 달러보다 큰 사람의 이메일을 가져오시오. (중복을 제거해야 한다.)
...
-- 그룹바이에서의 결과를 가지고 조건을 걸때는 where가 아니라 having으로 해야한다.
